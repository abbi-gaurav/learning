...
public class MyServer {
	...
	public MyServer(JettyServerParameters jettyServerParams)
	...
	private Server initConfig() throws FileNotFoundException, SAXException,
	...
	private void addConnector() throws FileNotFoundException, SAXException,
			IOException, Exception {
	...
	private void setLogging(boolean setLog) {
		if (setLog) {
			System.setProperty("org.mortbay.log.class",
					"com.test.util.log.MyLogger");
			// setting debug as true
			System.setProperty("DEBUG", "true");
		}
	}

	private Context addServlets(
			Map<String, Class<? extends HttpServlet>> servletMap) {
		Context ctx = new Context();
		ctx.setContextPath(jettyServerParams.getContextPath());
		ServletHandler servletHandler = new ServletHandler();

		for (Map.Entry<String, Class<? extends HttpServlet>> entry : servletMap
				.entrySet()) {
			servletHandler.addServletWithMapping(entry.getValue(),
					entry.getKey());
		}
	...
	private void configureServer() throws FileNotFoundException, SAXException,
			IOException, Exception {
		// XmlConfiguration configuration = new XmlConfiguration(new
		// FileInputStream("E:\\gaurav\\srcCode\\non-official\\jetty6.1\\jetty-6.1.4\\jetty-6.1.4\\etc\\jetty.xml"));
		// setupSSL();
		WebAppContext webAppContext = new WebAppContext();
		webAppContext.setTempDirectory(new File(
				"/home/gauravabbi/official/issues/helloworld_warIssue/tmp"));
		// webAppContext.setContextPath("/helloworld");
		// webAppContext.setWar("E:\\gaurav\\java\\J2EE\\jetty\\wars\\TestWeb1.war");
		webAppContext
				.setWar("/home/gauravabbi/official/issues/helloworld_warIssue/helloworld.war");
		...
		// webContext2.setTempDirectory((new
		// File("E:\\gaurav\\java\\J2EE\\jetty\\wars\\tmp7")));
		...
		// webContext3.setTempDirectory((new
		// File("E:\\gaurav\\java\\J2EE\\jetty\\wars\\tmp10")));
	...
	private void setupSSL() {
		...
		sslConnector
				.setKeystore("/Users/gabbi/work/buildup/downloads/server/serverPrivStore.jks");
		// sslConnector.setKeystoreType("PKCS12");
		sslConnector.setKeyPassword(PASSWORD);
		sslConnector
				.setTruststore("/Users/gabbi/work/buildup/downloads/server/serverTrusted.jks");
		...
		sslConnector.setProtocol("TLSV1");
	...
	private static void add_perfDartBoardWar() throws Exception {
		 ...
		 * WebAppContext("E:\\gaurav\\official\\devlopment\\jett6.xupgrade\\war"
		 * ,"/");//{
		 */WebAppContext context = new WebAppContext(
				"E:\\gaurav\\java\\J2EE\\jetty\\wars\\TestWeb2.war", "/") {
		...
		/*
		 * context.setTempDirectory((new
		 * File("E:\\gaurav\\java\\J2EE\\jetty\\wars\\pertfDartBoard")));
	...
	private static void addServlet2(Server server) {
		...
		ctx.setContextPath("/");
		ServletHandler servletHandler = new ServletHandler();
		// org.mortbay.jetty.plus.servlet.ServletHandler servletHandler = new
		// org.mortbay.jetty.plus.servlet.ServletHandler();
		// servletHandler.addServlet(new ServletHolder(TestServlet1.class));
		servletHandler
				.addServletWithMapping(TestServlet1.class, "/servlet_new");
		servletHandler.addServletWithMapping(TestServlet1.class,
				"/servlet2_new");
		...
		servletHandler.addServletWithMapping(TestServlet1.class,
				"/reflect-raw_new");
		servletHandler.addServletWithMapping(SpitServlet.class,
				"/httpperf/TestServlet_new");
		ctx.setServletHandler(servletHandler);
		server.addHandler(ctx);
	}
